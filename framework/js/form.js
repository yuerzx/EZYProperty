...
*修改者：钟晶晶
*修改时间：2012-12-22
*修改功能：增加了select下拉菜单选项动态更新功能
*使用方式：$('select').html('<option value="小柠檬">小柠檬</option><option value="xiaoningmeng.com">xiaoningmeng.com</option>').trigger('update');
******************************************** */
    ...
    ***************************/
    $.fn.jqTransCheckBox = function () {
        return this.each(function () {
            ...
            var aLink = $('<a title="" href="#" class="jqTransformCheckbox"></a>');
    ...
    ***************************/
    $.fn.jqTransRadio = function () {
        return this.each(function () {
            ...
            var aLink = $('<a title="" href="#" class="jqTransformRadio" rel="' + this.name + '"></a>');
    ...
    ***************************/
    $.fn.jqTransTextarea = function () {
        return this.each(function () {
            ...
            strTable += '';
            strTable += '';
            strTable += '<table class="jqTransformTextarea" cellpadding="0" cellspacing="0" border="0"><tbody><tr><td id="jqTransformTextarea-tl"></td><td id="jqTransformTextarea-tm"></td><td id="jqTransformTextarea-tr"></td></tr><tr><td id="jqTransformTextarea-ml"> </td><td id="jqTransformTextarea-mm"><div></div></td><td id="jqTransformTextarea-mr"> </td></tr><tr><td id="jqTransformTextarea-bl"></td><td id="jqTransformTextarea-bm"></td><td id="jqTransformTextarea-br"></td></tr></tbody></table>';
            var oTable = $(strTable)
    ...
    $.fn.jqTransSelect = function () {
        if (this.length) {
            this.each(function (index) {
                ...
                $wrapper.css({ width: selectWidth, zIndex: 99 - index }).prepend('<div><span></span><a title="" href="#" class="jqTransformSelectOpen"></a></div><ul></ul>');
                var $ul = $('ul', $wrapper).css('width',$select.width()).hide();


                /* Now we add the options */
                $('option', this).each(function (i) {
                    var oLi = $('<li><a title="' + $(this).html() + '" href="#" index="' + i + '">' + $(this).html() + '</a></li>');
                    $ul.append(oLi);
                });

                $select.update(function () {
                    $ul.empty().css({ 'height': 'auto', 'overflow': 'hidden' });
                    $('option', this).each(function (i) {
                        var oLi = $('<li><a title="' + $(this).text() + '" href="#" index="' + i + '">' + $(this).text() + '</a></li>');
                        $ul.append(oLi);
                    });
                    $('a:eq(' + this.selectedIndex + ')', $ul).addClass('selected').click();
                    //css里这里ul超过150px则出现滚动条，所以这里直接用150来判断，本来打算插件里加个参数可以自定义，后来考虑到还是保持整个插件的统一好了。
                    ($ul.jqTransformGetDimension('height') > 150) && $ul.css({ height: 150, 'overflow': 'auto' }); //hidden else bug with ff
                });
                ...
                oLinkOpen.click(function () {
                    ...
                    if (!already_open && !$select.attr('disabled')) {
                        ...
                        function enoughSpaceBelow() {
                            var ulHeight = $ul.jqTransformGetDimension('outerHeight');
                            spaceBelow = $(window).height() - $wrapper.outerHeight() - $wrapper.offset().top;
                            return ulHeight < spaceBelow;
                        }
                        $ul.css({
                            width: $select.jqTransformGetDimension('width') + 28,
                        ...
                        if (!$wrapper.data('clone')) {
                            var $clone = $wrapper.clone().hide().insertBefore($wrapper);
